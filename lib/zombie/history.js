...
History = (function() {
  ...
  History.prototype._resource = function(url, method, data, headers) {
    var Path, jsdom_opts, referer, _ref, _ref1,
      _this = this;
    jsdom_opts = {
      deferClose: true,
      features: {
    ...
    switch (url.protocol) {
      ...
      case "file:":
        ...
        return this._browser.resources.request(method, url, data, headers, function(error, response) {
          var document, evt, onload;
          if (!error) {
            jsdom_opts.url = response.url;
          }
          document = JSDOM.jsdom(null, HTML, jsdom_opts);
          if (_this._browser.runScripts) {
            Scripts.addInlineScriptSupport(document);
          }
          _this._window.document = document;
          document.window = document.parentWindow = _this._window;
          _this._window.navigator.userAgent = _this._browser.userAgent;
          document.addEventListener("DOMContentLoaded", function(event) {});
          onload = document.createEvent("HTMLEvents");
          onload.initEvent("load", false, false);
          _this._browser.dispatchEvent(_this._window, onload);
          if (error) {
            document.open();
